...
exports.generateBulkMarkdownReport = generateBulkMarkdownReport;
exports.generateJsonReport = generateJsonReport;
exports.generateBulkJsonReport = generateBulkJsonReport;
 ...
 */
function appendTimeline(lines, report) {
    if (report.timeline) {
        lines.push('---');
        lines.push('');
 ...
 */
function generateBulkMarkdownReport(report) {
    const lines = [];
    ...
    const filesWithSuggestions = report.fileReports.filter(f => f.suggestions.some(s => s.recommendation === 'BACKPORT'));
 ...
 */
function generateBulkJsonReport(report) {
    return JSON.stringify(report, null, 2);
}
