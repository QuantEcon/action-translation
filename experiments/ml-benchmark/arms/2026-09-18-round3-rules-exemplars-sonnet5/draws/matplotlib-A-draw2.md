...
Matplotlib scientific computing-നായി design ചെയ്ത ഒരു outstanding graphics library ആണ്, ഇതിൽ ഉള്ളവ:

* high-quality 2D and 3D plots
* usual formats-ലെല്ലാം (PDF, PNG, etc.) output
* LaTeX integration
* presentation-ന്റെ എല്ലാ aspects-ന്റെയും മേൽ fine-grained control
* animation, etc.

### Matplotlib's Split Personality

Matplotlib വ്യത്യസ്തമാകുന്നത് plotting-നായി രണ്ട് വ്യത്യസ്ത interfaces provide ചെയ്യുന്നു എന്നതിലാണ്.

ഒന്ന് MATLAB refugees-ന് ഒരു ready home കണ്ടെത്താൻ സഹായിക്കാൻ എഴുതിയ simple MATLAB-style API (Application Programming Interface) ആണ്.
...
ഇത് simple ഉം convenient ഉം ആണ്, പക്ഷേ കുറച്ച് limited ഉം un-Pythonic ഉം ആണ്.

For example, function calls-ൽ, പല objects-ഉം programmer-നെ അറിയിക്കാതെ create ചെയ്യപ്പെടുകയും pass ചെയ്യപ്പെടുകയും ചെയ്യുന്നു.

Python programmers സാധാരണയായി കൂടുതൽ explicit ആയ ഒരു programming style ആണ് preference ചെയ്യുന്നത് (ഒരു code block-ൽ `import this` run ചെയ്ത് രണ്ടാമത്തെ line നോക്കുക).
...
കുറച്ചുകൂടി typing ഉണ്ടെങ്കിലും, objects-ന്റെ കൂടുതൽ explicit ആയ ഉപയോഗം നമുക്ക് better control നൽകുന്നു.

നമ്മൾ മുന്നോട്ട് പോകുന്തോറും ഇത് കൂടുതൽ വ്യക്തമാകും.
...
Line-നെ അല്പം transparent ആക്കാൻ `alpha` ഉപയോഗിച്ചിട്ടുണ്ട്---ഇത് അതിനെ smooth ആയി കാണിക്കുന്നു.

`ax.legend()`-ന് പകരം `ax.legend(loc='upper center')` ഉപയോഗിച്ച് legend-ന്റെ location മാറ്റാം.
...
Matplotlib-ന് ഒരു huge array of functions and features ഉണ്ട്, ആവശ്യം വരുന്നതനുസരിച്ച് കാലക്രമേണ ഇവയെ കണ്ടെത്താം.
...
മൂന്ന് normal densities randomly generate ചെയ്ത് അവയുടെ mean ഉള്ള ഒരു label ചേർക്കുന്ന ഒരു example താഴെ കാണാം:
...
Matplotlib 3D plots-ന്റെ കാര്യത്തിൽ നല്ല ഒരു job ചെയ്യുന്നു --- ഒരു example താഴെ കാണാം:
...
നിങ്ങൾ പതിവായി ഉപയോഗിക്കുന്ന ഒരു set of customizations ഒരുപക്ഷേ നിങ്ങൾ കണ്ടെത്തിയേക്കാം.

Axes origin-ലൂടെ കടന്നുപോകുന്നതും, ഒരു grid ഉള്ളതും നമ്മൾ സാധാരണയായി preference ചെയ്യുന്നു എന്ന് കരുതുക.

ഈ changes implement ചെയ്യുന്ന ഒരു custom `subplots` function build ചെയ്യാൻ object-oriented API എങ്ങനെ ഉപയോഗിക്കാം എന്നതിന്റെ ഒരു നല്ല example [Matthew Doty](https://github.com/xcthulhu)-യിൽ നിന്നും താഴെ കാണാം.

Code ശ്രദ്ധയോടെ വായിച്ച്, എന്താണ് നടക്കുന്നത് എന്ന് നിങ്ങൾക്ക് follow ചെയ്യാൻ കഴിയുമോ എന്ന് നോക്കുക:
...
1. `ax`-ന് ആവശ്യമുള്ള customizations ചെയ്യുന്നു, ഒപ്പം
1. `fig, ax` pair-നെ calling code-ലേക്ക് തിരികെ pass ചെയ്യുന്നു.

### Style Sheets

Matplotlib-ലെ മറ്റൊരു useful feature ആണ് [style sheets](https://matplotlib.org/stable/gallery/style_sheets/style_sheets_reference.html).
...
Style sheet set ചെയ്യാൻ ഇനി നമുക്ക് `plt.style.use()` എന്ന method ഉപയോഗിക്കാം.
...
`ggplot` എങ്ങനെയിരിക്കുന്നു എന്ന് താഴെ കാണാം:
...
List-ലുള്ള മറ്റ് styles experiment ചെയ്യാൻ ഈ function നിങ്ങൾക്ക് ഉപയോഗിക്കാം.
...
നിങ്ങളുടെ style sheets-നായി set ചെയ്യാവുന്ന ധാരാളം parameters ഉണ്ട്.

നിങ്ങളുടെ style sheet-നുള്ള parameters set ചെയ്യാം: 

1. സ്വന്തമായി ഒരു [`matplotlibrc` file](https://matplotlib.org/stable/users/explain/customizing.html) create ചെയ്ത്, അല്ലെങ്കിൽ
2. `plt.rcParams` എന്ന dictionary-like variable-ൽ store ചെയ്ത values update ചെയ്ത്
...
`.rcParams`-ലെ parameters മാറ്റിയ ശേഷം generate ചെയ്യുന്ന ഏത് plot-നെയും ഈ setting affect ചെയ്യും.
...
നിങ്ങളുടെ style default ആയി തിരികെ മാറ്റാൻ `default` style sheet വീണ്ടും apply ചെയ്യുക:
