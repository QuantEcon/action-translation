...
- **`ml_metrics.py` gains two lints**: a Malayalam plural built on a Latin-script singular, and two adjacent hyphen-suffixed `-ഉം` items with no comma. Calibrated against the three reviewed lectures and the round-4 draft with no false positives.
