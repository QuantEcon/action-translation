...
- **A sync PR no longer carries a `_toc.yml` entry for a new lecture the same run failed to deliver** (#156): TOC processing ran unconditionally after the per-file loop and never consulted the run's error set, so when a _new_ lecture failed — in every field instance the structural-parity guard correctly refusing a scrambled translation (lecture-python.myst#993, #1004, #1007) — the PR still listed it in the TOC, the target build failed under `-n -W` with `toctree contains reference to nonexisting document`, and because a new lecture series cross-references itself, every delivered sibling that cited the missing file broke with it. After the loop the orchestrator now works out which new files did not arrive — translation errored inside the loop, or the source content could not be fetched and the file never reached the loop (a separate path the action now classifies by probing the target: a 404 means nothing will be delivered under that name; any other answer keeps the entry, so an outage never removes a live one) — and removes their entries from any `_toc.yml` in the changeset. Existing files are never candidates: a failed _update_ leaves the old translation in place and its entry is still correct. The removal is byte-preserving by construction: QuantEcon TOCs use the zero-indent block style that js-yaml re-indents on a round trip, which would have turned a one-line removal into a whole-file rewrite (401 changed lines on the 204-line lecture-python TOC) that hides the change from the reviewer and flips back on the next verbatim sync — so the entries' lines (with any `title:`/`sections:` children) are spliced out of the original text and the result is verified by parse (surviving `file:` list = original minus the dropped stems) before a parsed-and-re-serialised fallback is even considered. The PR body gains a "Files Failed to Translate" section listing the undelivered files and stating exactly what happened to their entries — removed from the named TOC, or nothing removed because no TOC was in the run, in which case a pre-existing target entry may still dangle and the reviewer is told so. The `translation-sync-metadata` block still declares the failed file, deliberately: the next rebase should retry it, and W1's declared-vs-delivered assertion (#259) is what will make a nine-declared/eight-delivered PR visible when no TOC is involved. Renamed files whose source fetch fails are not classified — dropping the new stem would orphan the old file from the toctree, which fails the strict build just the same; that case stays a run failure.

### Added

- **Malayalam: the second inline native-review round is encoded — ml glossary v0.4.0 (+16 terms), the rule set grown 18 → 24, and the first deterministic ml lints** (#296, #189 Phase 2 round 2): the editor of record reviewed `functions` on lecture-python-programming.ml#7 — 118 committable suggestion blocks on 118 of 123 Malayalam prose lines, all applied and every one dispositioned in QuantEcon/project-translation `reports/2026-09-01-ml-functions-review-disposition.md`. The headline is that the round-1 rules held (no recurrence of below-deixis, ordinal, light-verb, `-യുടെ` or suffixed-English flags), so this change encodes the _new_ classes the round surfaced, most of them deterministic: terminal punctuation (a colon on every paragraph that introduces a code cell, list or equation, a full stop on every other — 26 flags, regardless of the English source's punctuation); sentence-initial capitalisation of a retained-English word (`Functions-നെ`, `Built-in`, `Return statement` — 14); the hortative teacher's voice (`നമുക്ക് … -ആം`, never the plain future `നമ്മൾ … -ും` — 12); consistent accusative `-നെ` on light-verb objects with no additive `-ഉം` on list items (15); the fixed renderings he corrected on every occurrence ("a given" → `തന്നിരിക്കുന്ന`, "consider" → `നോക്കാം`, spelling `കുറച്ചുകൂടി` — 10); the sandwich-sentence rule (state a claim interrupted by code cells once, before the first cell); three more sentence-initial connectives kept English ("In particular", "Obviously", "Basically"); worked examples for the pointer sentences the model missed three times; and the maintainer's 2026-09-01 ruling that exercise statements with probabilistic or mathematical content stay English, hybrid sentences included (decision record `.dev/decisions/D-2026-09-01-ml-exercise-statements-stay-english.md`). Glossary: the ordinary words the editor kept moving back to English (`useful` ×4 — never ഉപയോഗപ്രദമായ — `provide`, `exist`, `automatically`, `create`, `improve`, `full`, `stage`, `answer`, `successive`), the `line`/`lines` pin the #273 regeneration flagged as a 14× variance regression, and four everyday words he moved the other way (`straightforward` → എളുപ്പം, `contrived` → കൃത്രിമം, `limited` → പരിമിതി, `facilitate` → സഹായിക്കുക). `experiments/ml-benchmark/scripts/ml_metrics.py` gains the round's lints as a LINT bucket (never FAIL): terminal punctuation, lowercase-Latin-initial sentences, a banned-renderings list drawn from the rules, and a future-hortative watch — string checks that sidestep #240 and #229, calibrated so the editor's reviewed text is clean (seed 24/19/16/6 → reviewed 1/1/2/1, the residuals being forms he himself kept) and the pre-review seed lights up. Held for the editor's answers on lecture-python-programming.ml#12: "For example" as a discourse rule and glossary pins for `already`, `name`, `example(s)`, `work`. ml-scoped only; no other language's rules move.

## [0.26.0] - 2026-08-17

### Added

- **Malayalam: the first inline native-review round is encoded as engine rules — ml glossary v0.3.0 (+4 terms) and the rule set grown 10 → 18** (#189 Phase 2): the editor of record reviewed `python_by_example` end-to-end on lecture-python-programming.ml#1 — 100 inline comments, every one applied to the seed and dispositioned in QuantEcon/project-translation `reports/2026-08-17-ml-python-by-example-review-disposition.md` — and this change encodes everything that round resolved to a rule or glossary entry, the ml-packet pattern (#237) applied to its first field data. Glossary: the instructional verbs his flags moved back to English (`cover` — never കൈകാര്യം ചെയ്യുക, which reads managerial; `break down`; `refer`; the adverb `repeatedly`), each pinned with per-flag provenance. Prompt rules, all field-derived: the below-deixis pattern (English "here is X" pointing at content that follows renders as `താഴെ കാണാം`, never a literal ഇവിടെ — 13 flags, the round's most consistent correction); discourse formulas rendered by function ("In fact,"/"On the other hand," stay English; "in other words" → അതായത്; sentence-initial "Now" → ഇനി; "almost always" → മിക്ക സമയത്തും); keep-English extended above the term level (short pointer sentences, majority-technical/parameter sentences, parenthetical language comparisons like "(as in C, Java or Go)"); morphology the reviewer corrected five-plus times (adjectival ordinals ആദ്യത്തെ/അവസാനത്തെ, ablative നിന്നും, dative -ഇനും, accusative -നെ on light-verb objects, genitive allomorph -യുടെ after vowel-final roots); the everyday-verb counter-direction (a line "ends" അവസാനിക്കണം, not end ആകുന്നു — the light-verb pattern is for technical actions only, and "is required" is ആവശ്യമായിവരുന്നു, never ആവശ്യപ്പെടുന്നു); a classroom sentence-splitting register rule; the F1 `ഉള്ള` identifying-modifier rule and the no-double-comparative constraint, both carried from the #252 follow-up round as the queued v0.3 payload; and the ruling that exercise Hint/Solution sections which are predominantly mathematical reasoning stay wholly in English (editor-proposed at flags 99–100, accepted by the maintainer 2026-08-17 — decision record `.dev/decisions/D-2026-08-17-ml-math-heavy-sections-stay-english.md`). Because the review judge injects `additionalRules` as policy (do-not-flag-compliance framing), the same change carries the #250 resolution: an explicit rule states that English-retained terms with Malayalam case-suffixes (`relationships-ൽ`) and English headings are the REQUIRED compliant form — the recurring `minor/terminology` demerit that editor-routed 8 of 9 clean ml harness syncs was a reviewer false positive against the policy, confirmed twice over (the editor's #250 ruling, and zero such flags across his 100-comment round). ml-scoped only: no other language's rules move, and the Stage 4 shadow-window freeze (cross-language rubric, floors) is untouched. Harness-first applies before this reaches a release: regenerate `python_by_example` and diff against the editor's reviewed text to confirm the rules generalise.
...
- **`claude-opus-5` calls no longer report $0.000, and an unpriced model warns instead of silently hiding spend** (#230): the glossary scripts' `PRICING` table lacked a `claude-opus-5` entry, so every call priced at zero — #228 Stage 1 ran six Opus 5 lectures whose real ~$0.4–0.5/lecture cost was invisible, under-reporting the stage by ~3x. The table gains `claude-opus-5` ($5/$25 per MTok), `claude-fable-5` ($10/$50), and `claude-haiku-4-5` ($1/$5) — prices confirmed against Anthropic's current model documentation, not recalled — and `costUSD` now warns once per unknown model while still returning zero, because zero is a legitimate value (cache-only responses) and must not double as an "unknown" sentinel. `VALID_MODEL_PATTERNS` in `src/models.ts` gains `claude-opus-5` and `claude-fable-5` in the same change, since both gaps had the same cause.
- **The scaffolded resync gate matches the action's parser: `startsWith`, not `contains`** (#234 item 4): the generated sync workflow fired on any comment _containing_ `\translate-resync` while the action requires the trimmed body to _start with_ it — so a comment merely quoting the command mid-text started a secrets-bearing runner that then no-op'd, and the mention author got silence rather than a resync. The gate, every doc and example that quotes it, and the E2E harness template now use `startsWith(github.event.comment.body, ...)`; the FAQ and action reference state the command-first rule explicitly. One residual asymmetry, accepted: the action trims leading whitespace and GitHub's `startsWith` does not, so a comment beginning with spaces before the command no longer triggers the workflow — command-first-at-column-one is now the contract the docs teach.
- **`docs/user/heading-maps.md` documents the key format the action actually writes** (#91): the page taught lowercase-hyphenated MyST-style IDs (`model-description: "模型描述"`) — a format no writer has ever produced, and which nearly re-encoded itself into the #86 evaluator rubric. Keys are the English heading text with `#` markers and MyST roles stripped and nothing else changed (`Model Description: "模型描述"`), and nested headings use `::` path keys at any depth (`Model Description::Assumptions`), which the page previously never mentioned. The wrong examples in the FAQ, the fresh-setup tutorial, and AGENTS.md are corrected in the same pass; `connect-existing.md` already showed the real format.
- **`translate init` writes seeded lecture files that end with a newline** (sibling of #116): the same defect #116 reported on the forward path lived on the init writer too — the translator trims the model's response and neither `applyTypography` nor `injectHeadingMap` restores the terminator, so every file `translate init` seeded was written with its last line unterminated, exactly the state that put `\ No newline at end of file` into all 69 Track B diffs. Found while fixing #116 (recorded there as out of scope), fixed the same way: `ensureTrailingNewline` applied when `finalContent` is computed, before the structural-parity guard, so the guard checks the exact bytes that get written — the same ordering forward's finalize path uses. The helper is shared from `commands/forward.ts` rather than duplicated; when #172's `finalizeTranslatedDocument` choke point lands, both callers collapse into it.
- **`translate forward` writes lecture files that end with a newline** (#116): the translator trims the model's response, and nothing on the forward path put a terminator back, so every resynced file was written with its last line unterminated — the Track B wave put `\ No newline at end of file` in all 69 PR diffs on `lecture-python.zh-cn`, which dirties the next edit's diff on each of those files and trips POSIX-minded tooling. The newline is added in `finalizeResyncContent`, which already owns the exact bytes that get written (the structural-parity guard runs on its output), so both output routes — the local write and the `--github` commit — inherit it from one place rather than each write site remembering; `--test` mode terminates its mock output the same way so the smoke path exercises the same bytes. Narrow by construction: content that already ends with a newline is byte-identical, trailing blank lines are preserved rather than collapsed, and empty content stays empty so it remains falsy and skips the write. The action's sync path was never affected — `reconstructDocument` has always ended with `.trim() + '\n'` — and `translate init` carried the same defect, fixed in the sibling entry above.
- **`engineVersion` in the verdict block no longer asserts a release that did not produce the verdict** (#244): `package.json` on a branch carries the previous release's number until the release commit bumps it, so every `@main`-produced verdict — including every harness verdict ever written, the exact place pre-release behaviour is meant to be observed — was stamped with the wrong version (all five ml calibration verdicts said `0.24.0` while their run logs showed the post-#237 glossary). The fix is runtime, not build-time: the runner sets `GITHUB_ACTION_REF` for the executing action step, so when that ref is not a release-shaped tag the version is suffixed with it as semver build metadata (`0.25.0+main`; characters invalid in build metadata are replaced with `-`, so a `feat/x` ref suffixes as `+feat-x` and the suffix need not equal `engineRef`), and a new optional `engineRef` field carries the ref verbatim (`v0`, `main`, a pinned SHA — the last giving exact tree attribution for free). The `unknown` sentinel is never suffixed — the contract documents the exact string and `engineRef` already carries the ref, so `unknown+main` would break consumers keying off it for zero information gain. Build-time embedding was considered and rejected: it would break the committed bundle's byte-identical rebuild check and embed the parent commit's SHA (the bundle at commit C is necessarily built before C exists). Field addition only — consumers ignore unknown fields per the contract's versioning rules, so no `schemaVersion` bump; `docs/user/metadata-contract.md` documents both changes. Release-tag checkouts (`v0`, `v0.25`, `v0.25.0`) keep the bare version, which was always truthful there — production verdicts are unaffected.
...
- **Docs tell the current truth, hand-maintained counts are gone, and the shipped bundle loses 2.1 MB** (#168, audit F47/F117/F129/F131/F132/F133/F134/F130/F68/F108/F69): the public README sat at v0.16.0 with "1,000+ tests (40 suites)" — seven releases stale, because the release checklist named only three of the seven files carrying counts. Exact test/suite counts are now **removed from prose** (README, docs/index, CONTRIBUTING, architecture, testing.md all say "run `npm test`"; the one hedged count stays in copilot-instructions), and the two divergent module maps are collapsed to **one** — `docs/developer/architecture.md`, completed with the ten modules both maps omitted and locked by a structural test that fails when any source module is missing from it. `src/cli/README.md`, which documented a nonexistent `resync` binary and 1 of 8 commands, is now a three-line pointer to the maintained reference. Glossary counts are corrected and de-contradicted everywhere (zh-cn 357, fa 357, **fr 364**, **ml 52**; the fr file contradicted itself and docs/index disagreed with itself two ways), the phantom top-level `language` key is gone from the schema example, and the stale `init` transcript line shows the loader's real output. The five undocumented CLI flags get rows and semantics notes — including `-j/--parallel`, the concurrency and cost-burst control on both bulk LLM commands, and the flag → config → `en` resolution that explains why `--source-language` has no commander default on `backward`/`forward`. **Bundle**: the committed sourcemap drops `sourcesContent` (2.85 MB → 0.76 MB; the `run.cjs` shim still decodes frames to `src/` paths — re-verified) and the `dist-action/glossary/` copy loop is deleted along with the copies, which nothing ever read: the runtime resolves the checked-out repo's `glossary/`, and the docs that taught the wrong model now say so.
- **Contract constants have one owner, and the labels the pipeline gates on are bootstrapped and checked** (#162, audit F120/F8/F42/F32/F141): strings that cross a process or repo boundary — the `action-translation` trigger label, the PR label sets, the failure-issue label, the `auto-merge-mode` enum — were spelled independently in TypeScript (ten literals across five files), `action.yml`, and the workflow templates, with nothing asserting producer and consumer agree; the last drift in this class merged six PRs of real translated content unreviewed. New `src/contracts.ts` owns them (generalising the `branch-naming.ts` pattern that has held since #115, with `DEFAULT_CLAUDE_MODEL` re-exported so `models.ts` stays the model's owner), every code consumer imports from it, and two structural tests enforce the contract: a confinement scan that fails when any source file re-spells a label literal (the npm package name shares bytes with the trigger label and is explicitly allowlisted as a different concept), and a parity check that `action.yml`'s `pr-labels`/`claude-model` defaults, the canonical review workflow's two gate clauses, and the written spec in `docs/user/metadata-contract.md` (which now also names the previously-omitted `translation-sync-failure` label) all agree with the constants. **Bootstrap and detection close the loop**: `translate forward --github` hard-requires three labels no code path ever created — `translate setup` now creates all pipeline labels idempotently (`gh label create --force`; target-repo set plus the failure-issue label in the source repo, with manual fallback commands on failure), `translate doctor --check-gh` verifies they exist and prints the create commands for any missing, `doctor` also warns when a workflow pins a fixed action version instead of the floating `@v0`, and sync mode warns when a custom `pr-labels` omits the trigger label — a config that silently produces unreviewed PRs. Label-application failure in the Action's PR path deliberately stays non-fatal per the audit-boundaries record; the bootstrap is the fix.
- **One canonical review-workflow template, and `translate setup` scaffolds repos that actually work** (#161, audit F7/F136/F27/F91/F135/F137): the review workflow existed in six divergent copies — the scaffolder plus five doc pages — and five of them could never fire on Action sync PRs, because the `action-translation` label is applied _after_ the PR opens and `types: [opened, synchronize]` never sees it. That is the shape behind the v0.21.0 incident where six PRs of real translated content merged unreviewed. The scaffolder's copy was also pinned to `v0.9.0` (25 releases stale, the release checklist never knew to bump it) with no `permissions:` or `concurrency:` and `checkout@v4` against the docs' `@v7`. **Now**: `examples/review-translations.yml` is the single canonical template — `translate setup` renders it (substituting `source-repo`/`source-language`/`docs-folder`) and every doc page quotes its shape, locked by a drift test in the `branch-naming` mold that fails when any copy loses the `labeled`/`reopened` triggers, the label guard, `permissions`, or the per-PR `concurrency` group. The scaffolder emits the floating `@v0` (nothing left for a release to forget), also writes `rebase-translations.yml` from its canonical template, and its source workflow gains the `issue_comment` resync trigger and composite guard the docs teach — plus the `g`-flag fix in the paths-filter normalizer, which emitted a broken `lectures//**/*.md` for `./lectures/`. `translate doctor` now warns on a review workflow whose trigger list lacks `labeled` instead of passing the repo.
...
- **`translate init` now refuses to write structurally corrupt translations** (#159, audit finding F1 — rank 1 of 139): `init` was the one model-output write path without the structural parity guard the sync and `forward` paths gained in v0.20.0 — and it is the path that seeds an entire edition, when corruption is most expensive and least likely to be noticed. A corrupt seed becomes the baseline the sync guard compares _forward_ from, so it can never be re-examined. `translateLecture` now runs `checkStructuralParity` on the exact bytes it would write and fails the file loudly (counted in the run's failure report) instead of writing, exactly like `forward`. Files that previously "succeeded" with dropped anchors or mutated directives will now fail — that is the guard working, not a regression. Measured over the seeded corpus before shipping (248 source/target pairs, five editions): 31 existing files would fail today's check, 25 of them in `lecture-intro.zh-cn` (seeded pre-guard; largely wholesale directive-count mismatches against today's source), with the other four editions nearly clean (6/199 — including the known `about_py.fa` and `jax_intro.zh-cn` damage).
...
- **CLI smoke tests fail on a stale build instead of validating it** (#158): the guard checked only that `dist/cli/index.js` _exists_, so the 11 smoke tests exercised whatever the last build left behind. It now also fails, with a `run npm run build` message, when any non-test file under `src/` is newer than the binary.
...
- **The dead `toc-file` input is deleted, and the docs stop promising any language works** (#167, audit F12/F67/F71/F102/F128/F126): `toc-file` was declared in `action.yml` with a documented default, parsed into `ActionInputs.tocFile`, asserted by a unit test — and read by nothing; the TOC filename has always been the hardcoded `_toc.yml`. One of the published interface's 11 documented inputs was a lie, found independently by four audit passes. Deleted (not wired) from all five surfaces. Separately, three user docs promised that _any_ language code works as `target-language` while the Action fails validation on anything outside `LANGUAGE_CONFIGS` (currently `en`, `zh-cn`, `fa`, `fr`, `ml`) before any work happens. The docs now state the real contract — Action validates, CLI genuinely accepts any code — the FAQ's stale zh-cn/fa-only enumeration is refreshed, and a drift guard in `language-config.test.ts` parses the supported-language marker out of `docs/user/language-config.md` and fails when it disagrees with `getSupportedLanguages()`.
- **~900 LOC of dead code, deleted by name under the type-checking PR A installed** (#166, audit F45/F103/F46/F104/F76/F105/F75/F99/F106/F115): the abandoned per-section backward pipeline (`buildEvaluationPrompt`, `parseEvaluationResponse`, `mockEvaluationResponse`, `evaluateSection` — the last with zero callers anywhere, deleted by function name per the boundaries record, preserving the four live helpers that sit among them: `validateCategory`, `parseSpecificChanges`, `sleep`, `buildSectionPairsBlock`); the legacy section-by-section forward-resync branch, dead on arrival — nothing ever constructed a `ResyncSectionResult`, so `buildForwardPRBody`'s section breakdown, the `sectionResults` parameters, both types, and the `sections: []` field every one of its nine producers wrote as a literal empty array are gone; four exported types dead everywhere (`BulkBackwardReportData`, `SpecificChangeData`, `TriageResultData`, `SyncResult`); and `ink-testing-library`, declared with zero imports, advertising a capability the CJS test config cannot provide. **The six tests guarded on a gitignored `reports/` fixture that exists on no machine — never run, could not fail — now run unconditionally** against a committed anonymised fixture (`src/cli/__tests__/fixtures/resync-reports/`). Headline test count drops 1,380 → 1,356 (~1.7%, the predicted optical drop): coverage measured before and after is flat-to-up (Lines 84.6% → 84.8%, Functions 86.3% → 86.4%), and the rebuilt `dist-action/` bundle is byte-identical — independent confirmation nothing deleted was reachable. `translateSectionResync` (live sync-mode path) untouched, per the boundaries record.

### Fixed

- **The write path stops pretending to validate itself, and nothing fails open anymore** (#165, audit F2/F121/F38/F54/F84/F85): `validateMyST` wrapped `parseSections` in a try/catch — and `parseSections` has no throw on any path, so it was structurally incapable of returning `valid: false`; both sync-path gates that called it were provably dead code that read as protection and logged validation over corrupt output. **Deleted**, per the audit's recommendation, leaving `checkStructuralParity` as the single honest guard (the residual hole — a directive-free, anchor-free source gives parity nothing to compare — is accepted and recorded; the real replacement is the #94 Phase 2 round-trip invariant, which the boundaries record requires reformulating first). **Fetch errors now fail the run**: a transient 5xx on one file used to yield a PR silently missing that file plus a success comment and a green check — `fetchAllFileContents` returns `{files, errors}` and fetch failures count as processing errors (failed run + failure issue); rebase mode goes further and throws **before any branch reset**, the #160 rule, since force-pushing without a file the metadata says the PR carries would drop it. **CLI parsers fail closed** (F54/F38): the document comparator's keyword fallback read garbled prose — including "not in sync" — as `IN_SYNC` and silently skipped the file; forward-triage's keyword branches turned a model reasoning aloud about its enumerated verdict options into a silently skipped resync; and the backward evaluator fabricated `NO_BACKPORT` for every section on parse failure, indistinguishable from a clean file. The keyword fallbacks are deleted (both defaults were already correctly recall-biased), and the backward parser throws a typed `LlmResponseParseError` that its retry loop re-asks on — the same treatment the Action reviewer has always given parse failures. **Resume reads are validated** (F84): `_progress.json` now goes through `parseProgressCheckpoint` (its first production caller) with a logged warning instead of an unvalidated cast, and corrupt report sidecars are warned about and counted in the summary instead of silently shrinking the aggregate. **Corrupt state is repairable, not overwritable** (F85): `doctor` reports a state YAML that exists but cannot be read as `Corrupt` (fail) instead of `Missing` — which used to send users to `status --write-state`, and that command then silently overwrote the file with current source HEAD, erasing the real sync point; `--write-state` now refuses corrupt files, listing them, unless `--force`.
- **One retry predicate for all six Anthropic call sites, no more stacked retries, and runs report what they cost** (#164, audit F52/F78/F53/F88): commit `40d63ef` added the `overloaded_error` retry branch to the translator only — the reviewer's two streamed calls, which run on every translation PR over the same transport, never got it, so a single overload aborted the whole review and re-paid both prompts. The predicate now has one owner (`isRetryableAnthropicError` in `models.ts`, consumed at all six sites; the CLI's bounded JSON-analysis calls OR in the retryable-truncation arm locally, since a full-document rerun at the same cap cannot fit but a JSON analysis can). Separately, all six `new Anthropic()` constructors passed no options, so the SDK's default 2 retries silently multiplied the hand-rolled 3 into up to 9 HTTP attempts with compounded backoff while the log said `attempt N/3` — every client now sets `maxRetries: 0` and the loop owns the budget it declares. A mocked reviewer retry suite (the second SDK-mock suite in the repo) pins the overload retry, the non-retry of `BadRequestError`, and the constructor option. **Cost accounting** (F53/F88): both chokepoints — `TranslationService.callWithRetry` and the reviewer's — now accumulate `input_tokens`/`output_tokens`/call count at the point of return, so retried and discarded attempts are included, which per-result `tokensUsed` fields structurally miss. Sync and review runs emit three new action outputs — `input-tokens`, `output-tokens`, `api-calls` — plus one `API usage:` summary log line; previously an unattended Action run discarded its token counts entirely. The fuller F88 threading (per-result usage objects, cache counters) stays open for the prompt-caching work.
- **Seven small defects with live consequences** (#163, audit F93/F87/F74/F40/F70/F81/F79 — the "cheap correctness" batch): **(F93)** the CLI resync metadata builder template-concatenated the docs folder and emitted `lectures//pv.md` for the Action's own canonical `lectures/` form, so rebase's byte-exact overlap check never matched and a genuinely conflicting PR was silently left unrebased under a green run — paths now go through a separator-collapsing join, `buildForwardSyncMetadata` has its first tests, and the metadata contract states the canonical single-slash form. **(F87)** a model response truncated at `max_tokens` threw a bare `Error` that failed the retry predicate, discarding a whole file's paid backward analysis on first occurrence; truncation is now a typed `TruncatedResponseError` in the retryable union at all three live call sites (the audit-boundaries record rules out raising the cap instead). **(F74)** two hand-counted `additionalRules` offsets emitted duplicate rule ordinals in every zh-cn/fa/fr prompt on the new-section and full-document paths — fixed, and a rendered-prompt drift test now asserts contiguous 1..N ordinals across all five builders × four languages (it fails 6 ways on the pre-fix code). **(F40)** a review whose source content could not be fetched still paid for two model calls and could top its comment with a "✅ PASS" header; it now refuses before the first call with a named error. **(F70)** the PASS/WARN/FAIL rule is extracted as pure `computeVerdict` with its thresholds tested for the first time — including the previously-unasserted interaction where syntax errors hold a 10/10 review at WARN, and the characterization that a 6/6 review is FAIL because IEEE 754 puts `6×0.7+6×0.3` a hair under the WARN boundary. **(F81)** a model answering `"strengths": "clear"` instead of an array crashed the comment builder after both calls were paid for; it now coerces to `[]` like the neighbouring `syntaxErrors` guard. **(F79)** review-mode glossary formatting rendered `- "term" → ""` for terms the translator filters out, so the reviewer judged against terminology the translation was never given; the same filter now applies, and the characterization test that pinned the wrong behaviour asserts the skip instead.

### Added

- **Sync failure issues are closed when a later sync succeeds** (#160, audit F31): every failed sync opens a fresh issue — deduping at creation is check-then-act, which races under this repo's concurrency (refused in `D-2026-07-16`) — but nothing ever closed them, so transient failures accumulated open issues indefinitely. On success the action now finds open issues matching the failure title for that PR/language exactly (not by label, which is applied best-effort) and closes them with a comment linking the successful translation PR.
...
- **The verdict block cannot be forged through lecture content, and consumers must parse the last block** (found validating verdict v2 on the harness, before release): the reviewer summarises source text an attacker may influence, and its summaries and finding descriptions render verbatim into the review comment. Crafted content could induce the model to emit a literal `translation-review-verdict` block inside a description — and because that forgery lands _earlier_ in the comment than the real block, a first-match parser returned it, yielding `recommendation: "auto-merge"` against an attacker-chosen SHA and overriding a genuine `FAIL`/`editor` verdict. Reproduced against the pre-fix implementation. Two defences: the engine now neutralises comment openings in model-authored prose, so the only verdict marker in a body is one it wrote; and `parseReviewVerdict` takes the **last** block, since the builder always appends the real verdict at the end. **Contract requirement for downstream consumers**: parse the last block, and treat a malformed last block as `editor` — never fall back to an earlier one, which is exactly how a forgery wins. Documented in `docs/user/metadata-contract.md`.
- **Malformed model output cannot open the auto-merge gate** (adversarial audit of the gate, pre-release): `diffChecks` were truthiness-tested, so a quoted `"false"` passed all four checks, and the loop enumerated the payload's own keys, so an empty `diffChecks` object reported nothing failing at all; criterion floors were lower-bounded only, so a response on a 0–100 scale cleared every floor; a failed source fetch let a review score the target against nothing and call it clean; `minor`/`syntax` findings did not gate; `max-suggestions: 0` blinded the findings half of the gate; and a findings payload whose items were all unreadable (`[{}]`, `[null]`) returned an empty list marked well-formed, reading as a clean review. Each is now a gating condition, with regression tests plus a fuzz pass over the whole trust boundary. Also bounded: syntax and diff finding descriptions bypassed the field-length cap, and a pathological review produced a ~400 KB block against GitHub's 65,536-character comment limit, so the comment failed to post and the PR got no verdict at all.

### Fixed

- **Resync preserves localisation implemented as a data-source substitution — pinned deterministically, verified after the fact** (#107 validation wave, 2026-07-22): the localisation-as-ground-truth enumeration covered in-line localized values (label dicts, plot strings, rename maps, print/docstrings) — all measured surviving byte-for-byte on `inequality` and `heavy_tails` — but not localisation done by _replacing how a variable is derived_: `long_run_growth`'s hand-fixed `code_to_name` reads the target-only `country_code_cn.csv` and selects its `name_chinese` column, and the resync reverted it to the source's English derivation (legends regress to English country names). A strengthened prompt rule alone **demonstrably does not hold** — the reverted block reproduced on a re-run with the rule in place — so the fix is deterministic, in the parity-guard mold (`src/cli/target-local-reads.ts`): before the model call, forward scans the target's code cells for reads of data files that exist only in the translation repo (basename match across the two docs trees; URLs excluded) and pins those exact lines in the request's custom instructions; after finalization it verifies every pinned line survived and **fails the file loudly** if not — a silent revert merges English legends, a loud failure gets a human. The prompt also names the class (source-side data updates go _through_ the localized mechanism), but the guarantee comes from the pin + verify, not the prose.
...
- **`forward --github` PRs now carry the `action-translation` label, so the review workflow actually triggers on them** (#131): the CLI hard-coded its label set as `action-translation-sync` + `resync`, while the deployed review workflow template gates on `action-translation` — the label the _action's_ pr-creator applies. Every CLI resync PR therefore failed the filter and `Review Translations` completed as `skipped`, silently; all six PRs of the lecture-python.zh-cn post-wave drift mini-wave went unreviewed until the label was added by hand. The CLI now applies `action-translation` alongside its existing labels, keeping the template's label contract single-valued instead of widening the filter in every deployed copy.
...
- **Truncation detection**: every Claude call (translator, reviewer, backward evaluator, forward triage, document comparator) now checks `stop_reason` and fails the operation on `max_tokens` instead of using cut-off output — previously a truncated backward analysis parsed as `NO_BACKPORT` and reported the file as _clean_.
...
- **Dependencies**: Removed 10 unused AST-related packages (unified, remark-_, mdast-_, diff)
