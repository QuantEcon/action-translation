...
- **Thinking disabled tool-wide** (`DEFAULT_THINKING` in `src/models.ts`). Sonnet 5 runs adaptive thinking by default, which would bill thinking tokens on every bulk translation and place a thinking block first in the response — breaking the `content[0]` text extraction in the translator and reviewer. Disabling preserves the exact behavior the shipped action has always run (Sonnet 4.6 with no thinking param). This is a deliberate hold, not a quality verdict — thinking's value on Sonnet 5 is being measured before we enable it (see `experiments/thinking-sonnet5/PLAN.md`).
