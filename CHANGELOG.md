...
- **`translate forward` now ends generated lecture files with a newline** (#116): whole-file resync previously passed model or test output unchanged to both the local file writer and the `--github` commit path, so all 69 observed branches could produce Markdown without a POSIX trailing newline. Output is now normalized once at the shared boundary before either sink, preserving existing LF and CRLF endings while adding a single LF only when one is absent. Regression coverage exercises both local writes and GitHub-mode output.
