...
- **`actions/checkout` in `examples/` and `docs/` moved from `@v4` to `@v7`** — the templates were pinned older than any edition actually running them (v6 and v7 in production), so "mirrors the upstream template" was aspirational rather than true.
