...
- **`ml` rules 23 → 27, glossary v0.5.0 → v0.6.0** from the third inline native review (44 suggestion blocks on lecture-python-programming.ml#13, `matplotlib`; dispositions in QuantEcon/project-translation `reports/2026-09-18-ml-matplotlib-review-disposition.md`). Every class encoded from round 2 came through clean (terminal punctuation 26 → 0 flags, capitalisation 14 → 0, fixed pairs 10 → 0), and the editor left 22 of 66 Malayalam lines untouched against 5 of 123. New rules: clause-boundary commas (after a long topic phrase and always after എന്നത്; after a fronted adverbial, purpose, instrumental or conditional clause; between coordinated phrases), verb form by meaning (perfect-stative -തിരിക്കുന്ന for resultant states, -ഇട്ടുണ്ട്, conditional -ആൽ for "by doing X", -ഏക്കാം with ഒരുപക്ഷേ), idiom and coined jargon rendered by plain meaning, and the Further Reading scope ruling (decision record `D-2026-09-18-ml-further-reading-lists-stay-english`). Extended rules: the hortative covers reader-directed "think of it as" (കരുതാം); the teacher's-register rule covers fragment bullets, instrument-first ordering and the placement of മറ്റൊരു; the fixed renderings gain *multiple* → ഒന്നിലധികം, *explicit* → വ്യക്തമായ, *X-like* → X പോലെയുള്ള and *simple* never ലളിതമായ; the light-verb rule names *remove* and *label*. The hortative rule no longer ends on a dangling colon (#301). Glossary: *multiple*, *explicit* (Malayalam), *remove*, *label* (kept English); no `style_examples` (evaluated and set aside — see the arm entry). **Admission test from this round on**: a pattern becomes a rule only if it recurs — only 9 of 66 lines were identical between two draws of this lecture, so a singleton flag is as likely sampling noise as engine habit. Held for the editor's answers on lecture-python-programming.ml#22: the rule-2 boundary, *provide*, *prefer*, *draw*, the plural of retained nouns, and the comma between single-word -ഉം pairs.
- **ml round-3 regeneration arm archived** under `experiments/ml-benchmark/arms/2026-09-18-round3-rules-exemplars-sonnet5/`: rules × style examples on `matplotlib` (in-sample) and on a held-out lecture (`functions`), three draws per cell, plus three example variants (contrastive triples, a 56-pair set, the 56-pair set without the new rules) — 31 draws in all — scored two ways: paragraph similarity to the editor's text, and a **blind pairwise style judge** (`judge.mjs`, Opus 5, randomised order, 2,570 judgements, no position bias). Held out, the rules are preferred 55 : 45 over baseline and rules + the shipped pairs 57 : 43 (p = 0.054); every example variant judges dead even (49–51%), so no set ships and the line of inquiry is closed pending round-4 editor data. `ml_metrics.py` gains the round-3 banned renderings. Experiment data only.
...
- **Prompt caching on every translator API call** (#292, #293): each translator prompt is split into a stable block (static rules + language `additionalRules` + rendered glossary — byte-identical across a run) carrying `cache_control: {type: 'ephemeral'}`, and a volatile block (`customInstructions` + document content). Repeat calls in a run read the 10–12K-token prefix at the cached rate. All five builders are covered (section update, section resync, new section, full document, document resync); `customInstructions` moves to the head of the volatile block so the cacheable prefix is not broken. Semantics: per-result `tokensUsed` and the `input-tokens` output now exclude cached tokens — total prompt size is `input + cache-creation + cache-read`, as `action.yml` states. CLI triage/eval prompts are deliberately not cached (their rubric is under the 1024-token minimum) and the reviewer breakpoint is deferred.
- **`tool-review-injection/` — certify the reviewer by injected-defect catch rate** (#285, QuantEcon/project-translation#28): the review-mode half of the per-mode eval harness. Field observation cannot bound the auto-merge gate's precision (~1.2 eligible PRs/week means a ≤1% bound takes ~4.6 years), so this manufactures the positive class — defects injected into clean translations, reviewed, scored against the gate re-implemented in `lib.mjs`. First run (140 reviews of 45 zh-cn fixtures): M0-core catch rate 72%, 24 of 36 non-catches detected but filed below the gate, both negative controls false-flagged, routing flipped on 3 of 5 identical clean re-reviews. Analysis in QuantEcon/project-translation `reports/2026-08-20-m0-reviewer-certification.md`.

### Changed
- **`runReview` extracted from `src/index.ts` into `src/action/review.ts` and tested** (#278, #169 slice 1 — the W1 gate): the first slice of splitting the 1,579-line Action entry point that no test could import. Semantics-preserving by construction — the set of `core.setOutput` calls in the bundle is identical before and after, `action.yml`'s entry contract is untouched, and `import.meta.url` resolves through the same esbuild banner (so `../glossary` still resolves to the repo-root `glossary/`). The architecture module map gains the new modules.

### Fixed
- **E2E harness targets carry a `.translate/config.yml` again, with the production `editors:` block** (#275, QuantEcon/project-translation#24): the #202 reset deleted `.translate/` on every target reset and nothing re-created it, so the harness had been running with no translate config at all. The reset now lays the config down per target (block-style YAML, `primary:` first — the only form the parser reads and the shape `writeConfig`'s unknown-key merge preserves).
...
### Removed
- **The dead `toc-file` input is deleted, and the docs stop promising any language works** (#167, audit F12/F67/F71/F102/F128/F126): `toc-file` was declared in `action.yml` with a documented default, parsed into `ActionInputs.tocFile`, asserted by a unit test — and read by nothing; the TOC filename has always been the hardcoded `_toc.yml`. One of the published interface's 11 documented inputs was a lie, found independently by four audit passes. Deleted (not wired) from all five surfaces. Separately, three user docs promised that *any* language code works as `target-language` while the Action fails validation on anything outside `LANGUAGE_CONFIGS` (currently `en`, `zh-cn`, `fa`, `fr`, `ml`) before any work happens. The docs now state the real contract — Action validates, CLI genuinely accepts any code — the FAQ's stale zh-cn/fa-only enumeration is refreshed, and a drift guard in `language-config.test.ts` parses the supported-language marker out of `docs/user/language-config.md` and fails when it disagrees with `getSupportedLanguages()`.
