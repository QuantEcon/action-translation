...
export function generateMarkdownReport(report: BackwardReport): string {
  ...
  if (report.sourceRepo || report.targetRepo) {
    lines.push(
      `**SOURCE**: ${report.sourceRepo || 'unknown'} | **TARGET**: ${report.targetRepo || 'unknown'}`
    );
  }
  ...
  if (report.sourceMetadata) {
    lines.push(
      `**SOURCE last modified**: ${formatDate(report.sourceMetadata.lastModified)} by ${report.sourceMetadata.lastAuthor}`
    );
  }
  if (report.targetMetadata) {
    lines.push(
      `**TARGET last modified**: ${formatDate(report.targetMetadata.lastModified)} by ${report.targetMetadata.lastAuthor}`
    );
  }
  lines.push('');
  ...
  if (report.triageResult.verdict === 'IN_SYNC') {
    ...
    lines.push(
      `**Result: 📋 ${backportCount} SUGGESTION${backportCount === 1 ? '' : 'S'}** — See details below.`
    );
  ...
  if (report.triageResult.verdict === 'IN_SYNC') {
    lines.push(
      'The translation appears to be a faithful rendering of the source. No backport suggestions.'
    );
    lines.push('');
  ...
  if (report.triageResult.verdict === 'SKIPPED_TOO_LARGE') {
    lines.push(
      'Document was too large for single-call triage. Section-level analysis was performed directly.'
    );
    lines.push('');
  }
  ...
  const noBackportSuggestions = report.suggestions.filter(
    (s) => s.recommendation === 'NO_BACKPORT'
  );
...
function formatSuggestion(suggestion: BackportSuggestion): string {
  ...
  const confidenceLabel =
    suggestion.confidence >= 0.85 ? 'HIGH' : suggestion.confidence >= 0.6 ? 'MEDIUM' : 'LOW';

  lines.push(
    `### ${suggestion.sectionHeading} (${confidenceLabel} confidence: ${suggestion.confidence.toFixed(2)})`
  );
...
export function generateBulkMarkdownReport(report: BulkBackwardReport): string {
  ...
  const filesWithSuggestions = report.fileReports.filter((f) =>
    f.suggestions.some((s) => s.recommendation === 'BACKPORT')
  );
  ...
  if (filesWithSuggestions.length > 0) {
    ...
    for (const fileReport of filesWithSuggestions) {
      ...
      lines.push(
        `### ${fileReport.file} (${suggestions.length} suggestion${suggestions.length === 1 ? '' : 's'})`
      );
      lines.push('');
