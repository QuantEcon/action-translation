...
function normalizePathsFilter(docsFolder: string): string {
  const trimmed = docsFolder.replace(/^\.?\/+|\/$/, '');
...
/**
 * Generate the TARGET repository review workflow YAML.
 * Triggers on translation PRs (labeled action-translation) — posts quality review.
 */
...
export async function runSetup(
  ...
  if (options.dryRun) {
    ...
    return {
      ...
      localPath,
      filesCreated: ['.translate/config.yml', '.github/workflows/review-translations.yml', '.gitignore', 'README.md'],
      success: true,
    };
  }
  ...
  console.log(`     translate init -s /path/to/${options.source.split('/')[1]} -t ${localPath} --target-language ${options.targetLanguage}`);
  ...
  if (!options.sourceWorkflow) {
    ...
    console.log(`     Copy the workflow template from docs/user/cli-reference.md into`);
