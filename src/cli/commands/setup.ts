...
// Strip leading `./`//`/` and trailing slashes from a docs-folder value.
// The `g` flag and `\/+` matter: without them only one decoration comes off,
// and `./lectures/` or `lectures//` leaves a broken double slash behind.
function trimDocsFolder(docsFolder: string): string {
  return docsFolder.replace(/^\.?\/+|\/+$/g, '');
}
...
function normalizePathsFilter(docsFolder: string): string {
  const trimmed = trimDocsFolder(docsFolder);
...
export function generateSourceWorkflowYaml(
  ...
  // Echo the cleaned value into the input line too — a decorated folder like
  // `lectures//` should not survive into the scaffolded workflow.
  docsFolder = trimDocsFolder(docsFolder) || '.';
...
jobs:
  sync:
    ...
    # on a merged PR retry a failed sync. That path requires all three: a
    # comment on a PR (not a bare issue), the command, and a trusted author —
    # otherwise any account could fire a secrets-bearing run (Anthropic spend
    # plus the PAT) from any comment on the repo.
    if: >
...
export async function runSetup(
  ...
  const targetWorkflowContent = generateTargetWorkflowYaml(
    reviewTemplate,
    options.source,
    trimDocsFolder(options.docsFolder) || '.',
